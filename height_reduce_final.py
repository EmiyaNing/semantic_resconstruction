...
class Panoformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
                 embed_dim=32, depths=[2, 2, 2, 2], num_heads=[1, 2, 4, 8],
    ...
    def forward(self, x):
        ...
        # conv_val1 = torch.var(conv0, dim=2)
        # conv_val2 = torch.var(conv1, dim=2)
        # conv_val3 = torch.var(conv2, dim=2)
        # conv_val4 = torch.var(conv3, dim=2)
        # val_1= torch.var(pool0, dim=2)
        # val_2 = torch.var(pool1, dim=2)
        # val_3 = torch.var(pool2, dim=2)
        # val_4 = torch.var(pool3, dim=2)
        y=[]
        y.append(self.output_proj_0(pool0))
        y.append(self.output_proj_1(pool1))
        y.append(self.output_proj_2(pool2))
        y.append(self.output_proj_3(pool3))
        return y
...
class HorizonNet(nn.Module):
    ...
    def __init__(self,  use_rnn):
        ...
        self.feature_extractor=Panoformer()
